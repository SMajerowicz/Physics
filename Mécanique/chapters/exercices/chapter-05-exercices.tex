...
\benn
	...
	\end{cases}
\eenn
En supprimant les ordres de fr\'{e}quence sup\'{e}rieurs \`{a} $3(\omega_{0} + \epsilon/2)$, nous arrivons, en regroupant les termes, \`{a} :
\bea
...
Il s'agit ici de d\'{e}terminer le domaine d'instabilit\'{e} pour une r\'{e}sonnance autour de $\Gamma = \omega_{0}$, et non plus $\Gamma = 2\omega_{0}$ comme dans l'exercice pr\'{e}c\'{e}dent. Dans ce cas, l'\'{e}quation (\ref{EQ:27_8}) s'\'{e}crit :
\benn
...
\eenn
Comme pour l'exercice pr\'{e}c\'{e}dent, nous consid\'{e}rons les c{\oe}fficients constants pour faciliter le calcul des bornes du domaine de r\'{e}sonnance param\'{e}trique. L'\'{e}quation du mouvement devient :
\bea
...
Il s'agit ici de d\'{e}termine les conditions de la r\'{e}sonance param\'{e}trique dans le cadre du pendule d\'{e}crit dans l'exercice (\ref{PAR:5_EX3c}) dont la fonction de Lagrange calcul\'{e}e est :
\benn
...
\bea
	...
	\ddot{x}_{1} + 2\lambda\dot{x}_{1} + \omega_{0}^{2}x_{1} + \alpha x_{1}^{2} + \beta x_{1}^{3} & = & -\alpha x_{0}^{2} - 2\alpha x_{0}x_{1} - \beta x_{0}^{3} - 3\beta x_{0}^{2}x_{1} - 3\beta x_{0}x_{1}^{2} \nonumber
\eea
...
\begin{itemize}
	...
	\item $x_{0}^{2}x_{1}$ ne laisse pas de possibilit\'{e} pour $x_{1}$ car la fr\'{e}quence de $x_{0}^{2}$ est d\'{e}j\`{a} de $6\omega_{0}$
\end{itemize}
...
et cela nous donne :
\benn
	b^{2}\left[(\epsilon/3 - \xi b^{2})^{2} + \lambda^{2}\right] = \frac{1}{4m\omega_{0}^{2}}\frac{9\beta^{2}f^{2}b^{4}}{2^{10}m^{2}\omega_{0}{4}} = \frac{9\beta^{2}f^{2}}{2^{12}m^{2}\omega_{0}{6}}b^{4} = Ab^{2}
\eenn
dont $b = 0$ est une solution triviale. Pour $b \neq 0$, nous pouvons d\'{e}velopper ainsi :
\benn
