...
\bea
	L & = & \dfrac{m_{A}}{2}\vec{\dot{u}}_{3}^{2} + \dfrac{m_{B}}{2}\vec{\dot{u}}_{2}^{2} + \dfrac{m_{A}}{2}\vec{\dot{u}}_{1}^{2} - \dfrac{k_{l}}{2}(\delta l_{1}^{2} + \delta l_{3}^{2}) - \dfrac{k_{r}}{2}l^{2}\delta^{2} \nonumber \\
	...
	& & - \dfrac{k_{r}}{2}l^{2}\left[(x_{1} - x_{2})\cos\alpha - (y_{1} - y_{2})\sin\alpha - (x_{3} - x_{2})\cos\alpha + (y_{3} - y_{2})\sin\alpha\right]^{2} \nonumber
\eea
Maintenant, introduisons les coordonn\'ees suivantes : $Q_{a} = x_{1} + x_{3}$, $q_{s1} = x_{1} - x_{3}$ et $q_{s2} = y_{1} + y_{3}$ et \'ecrivons :
\benn
	\begin{cases}
		Q_{a} + q_{s1} = 2x_{1} \Leftrightarrow x_{1} = \dfrac{1}{2}(Q_{a} + q_{s1}) \\
		Q_{a} - q_{s1} = 2x_{3} \Leftrightarrow x_{3} = \dfrac{1}{2}(Q_{a} - q_{s1})
	\end{cases}
\eenn
L'utilisation de l'\'equation (\ref{EQ:24_EX2_1}) permet de conclure \`a :
\benn
	\begin{cases}
		x_{2} = -\dfrac{m_{1}}{m_{2}}(x_{1} + x_{3}) = -\dfrac{m_{1}}{m_{2}}Q_{a} \\
		y_{2} = -\dfrac{m_{1}}{m_{2}}(y_{1} + y_{3}) = -\dfrac{m_{1}}{m_{2}}q_{s2}
	\end{cases}
\eenn
et l'\'equation (\ref{EQ:24_EX2_2}) \`a :
\benn
	\begin{cases}
		-Q_{a}\cos\alpha + (q_{s2} - y_{3} - y_{3})\sin\alpha = 0 \Leftrightarrow y_{3} = \dfrac{1}{2}(q_{s2} - Q_{a}\cotan\alpha) \\
		-Q_{a}\cos\alpha + (y_{1} - q_{s2} + y_{3})\sin\alpha = 0 \Leftrightarrow y_{1} = \dfrac{1}{2}(q_{s2} + Q_{a}\cotan\alpha)
	\end{cases}
\eenn
