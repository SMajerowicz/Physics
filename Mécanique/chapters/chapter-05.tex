...
\benn
	U(q) = U(q_{0}) + U'(q_{0})(q - q_{0}) + \dfrac{U"(q_{0})}{2}(q - q_{0})^{2} \Leftrightarrow U(q) - U(q_{0}) = \dfrac{k}{2}(q - q_{0})^{2}
\eenn
...
\ee
Au regard des relations (\ref{EQ:4_1}) et (\ref{EQ:5_5}), l'\'energie cin\'etique pour une particule et dans le cas d'un unique degr\'e de libert\'e, s'\'ecrit $T = \frac{1}{2}a(q)\dot{q}^{\,2}$. Or dans le cadre des petites oscillations, nous avons \'{e}videmment $x \approx q$. De la m\^eme mani\`ere, $a(q) \approx q(a_{0})$, quantit\'e qui peut s'identifier \`a la masse de la particule si $x$ est une coordonn\'ee cart\'esienne. L'\'energie cin\'etique peut donc se formuler comme $T = \frac{1}{2}m\dot{x}^{2}$. Par cons\'equent, la fonction de Lagrange pour un syst\`eme r\'ealisant de petites oscillations, d\'enomm\'e aussi \emph{oscillateur lin\'eaire} est :
\be
...
\ee
Cette derni\`{e}re \'{e}quation peut \'egalement s'\'ecrire :
\be
	\ddot{x} + \omega^{2}x = 0 \label{EQ:21_5}
\ee
en ayant pos\'{e} la quantit\'e :
\be
...
\ee
o\`u $a$ est l'\emph{amplitude} et $\alpha$ la valeur initiale de la \emph{phase}, qui d\'epend de l'origine prise pour $\mathrm{t}$.Comme $\cos(\omega t + \alpha) = \cos(\omega t)\cos\alpha - \sin(\omega t)\sin\alpha$, alors nous pouvons en d\'eduire, \`a partir de la relation (\ref{EQ:21_7}), $c_{1} = a\cos\alpha$ et $c_{2} = -a\sin\alpha$. De plus :
\be
	\begin{cases}
		c_{1}^{2} + c_{2}^{2} = a^{2}(\cos^{2}\alpha + \sin^{2}\alpha) \Leftrightarrow a = \sqrt{c_{1}^{2} + c_{2}^{2}} \\
...
\ee
Comme $\epsilon \ll \omega$, l'expression $A + Be^{i\epsilon t}$ varie peu lors d'une p\'eriode $2\pi/\omega$. Au voisinage de la r\'esonnance, le mouvement se constitue de petites oscillations \`a amplitude variable. L'amplitude reste une grandeur r\'eelle aussi nous d\'efinissons $C = \lvert A + Be^{i\epsilon t} \lvert$. La relation (\ref{EQ:21_12}) permet d'\'ecrire $A = ae^{i\alpha}$ et $B = be^{i\beta}$, et ainsi l'amplitude $C$ devient :
\bea
	...
	\Leftrightarrow C^{2} & = & a^{2} + b^{2} + 2ab\cos(\epsilon t + \beta - \alpha) \label{EQ:22_7}
\eea
Les valeurs extr\^{e}mes de $C$ sont :
\begin{itemize}
...
\bea
	...
	\Leftrightarrow \dfrac{\mathrm{d}\xi}{\mathrm{dt}} - i\omega\xi = \dfrac{F(t)}{m} \label{EQ:22_8}
\eea
en d\'efinissant la quantit\'e $\xi$ telle que :
\be
	\xi = \dot{x} + i\omega x \label{EQ:22_9}
\ee
L'\'equation (\ref{EQ:22_8}) n'est plus du second ordre. Sa solution g\'en\'erale sans second membre peut s'\'ecrire comme $\xi_{0}e^{i\omega t}$ avec $\xi_{0}$ une constante d\'ependant des conditions initialies et \'egale \`a $\xi(t=0)$. Cherchons d\'esormais une int\'egrale particuli\`ere de la forme $A(t)e^{i\omega t}$ pour l'\'equation avec second membre. En ins\'erant cette solution particuli\`ere dans l'\'equation (\ref{EQ:22_8}) :
\bea
...
Dans le cas d'oscillations forc\'ees, l'\'energie totale du syst\`eme ne se conserve \'evidemmment pas. L'\'energie totale transmise au syst\`eme par la force ext\'erieure sachant que cette \'energie est nulle initialement, peut se calculer \`a partir de la relation (\ref{EQ:22_10}) :
\be
...
\eenn
aussi l'\'energie transmise est le carr\'e du module du c{\oe}fficient de Fourier de la force $F(t)$ avec une fr\'equence \'egale \`a la fr\'{e}quence propre du syst\`eme physique.
...
\eenn
Par cons\'equent, une force qui ne s'exprime que sur une faible dur\'ee communique au syst\`eme une impulsion $\int F\mathrm{dt}$ qui ne permet pas de provoquer de d\'eplacements notables, ce qui revient \`a notre hypoth\`ese de base des petites oscillations.
...
\bea
	...
	\Leftrightarrow \dfrac{\mathrm{d}}{\mathrm{dt}}\left(\dfrac{1}{2}\sum_{j}m_{ij}\dot{x}_{j}\right) + \dfrac{1}{2}\sum_{j}k_{ij}x_{j} & = & 0 \Leftrightarrow  \sum_{j}m_{ij}\ddot{x}_{j} + \sum_{j}k_{ij}x_{j} = 0 \nonumber \\
	\Leftrightarrow \sum_{j}(m_{ij}\ddot{x}_{j} + k_{ij}x_{j}) & = & 0 \label{EQ:23_5}
\eea
...
\eenn
o\`u les quantit\'es $k_{ij}$ et $m_{ij}$ sont r\'eelles et sym\'etriques. Donc :
\benn
	\left(\sum_{ij}k_{ij}A_{i}^{*}A_{j}\right)^{*} = \sum_{ij}k_{ij}A_{i}A_{j}^{*} = \sum_{ij}k_{ij}A_{j}A_{i}^{*}
\eenn
Ceci est aussi vrai pour le d\'enominateur de l'expression donnant $\omega^{2}$. En accord avec les d\'{e}finitions (\ref{EQ:23_2}) et (\ref{EQ:23_3}), $\omega^{2}$ a alors des formes d\'efinies positives au num\'erateur et au d\'enominateur. C'est donc pourquoi les valeurs de $\omega_{\alpha}^{2}$ ne peuvent \^etre que réelles et positives.

Les fr\'equences $\omega_{\alpha}$ \'etant connues, les \'equations (\ref{EQ:23_7}) permettent d'obtenir les valeurs $A_{j}$. Si les valeurs $\omega_{\alpha}$ sont toutes distinctes alors les c{\oe}fficients $A_{j}$ sont proportionnels aux mineurs\footnote{Le mineur est le d\'eterminant d'une sous-matrice carr\'ee.} de $k_{ij} - \omega^{2}m_{ij}$ en posant $\omega = \begin{Bmatrix}\omega_{\alpha}\end{Bmatrix}^{s}_{1}$. Soit $\Delta_{j\alpha}$ le mineur permettant le calcul de la quantit\'e $A_{j}$ avec pour fr\'equence $\omega_{\alpha}$, l'expression (\ref{EQ:23_5}) peut alors s'\'ecrire : $x_{j} = \Delta_{j\alpha}C_{\alpha}e^{i\omega_{\alpha}t}$ avec $C_{\alpha} \in \mathbb{C}$ et $\Delta_{j\alpha} \in \mathbb{R}$. Il s'agit alors d'une solution particuli\`ere puisque calcul\'ee pour une fr\'equence $\omega_{\alpha}$ donn\'ee. La solution g\'en\'erale est la somme des solutions particuli\`eres qui vaut dans $\mathbb{R}$ :
\be
...
\eenn
En pla\c{c}ant l'origine des coordonn\'ees au minimum de l'\'energie potentielle $U(x,y,z)$, une rotation des axes permet de rendre diagonale l'\'energie potentielle, i.e. $\frac{\partial U}{\partial x} = f(x)$, $\frac{\partial U}{\partial y} = g(y)$ et $\frac{\partial U}{\partial z} = h(z)$. En reprenant la formulation de l'\'energie potentielle d\'ecrite dans la relation (\ref{EQ:21_2}), la fonction de Lagrange devient :
\be
...
\ee
avec $L_{0}$ la fonction de Lagrange des oscillations libres, voir la relation (\ref{EQ:23_14}), et $F_{k}$, la force ext\'erieure appliqu\'ee sur la coordonn\'ee $x_{k}$. La formule (\ref{EQ:23_9}) nous donne :
\benn
...
Pour se faire, il est n\'ecessaire de se placer dans le r\'ef\'erentiel du centre d'inertie dans lequel l'impulsion totale du syst\`eme est nulle, voir le paragraphe (\ref{PAR:8}). Cela s'\'ecrit en reprenant la relation (\ref{EQ:8_3}) :
\benn
...
\eenn
En posant $\vec{r}_{a} = \vec{r}_{a0} + \vec{u}_{a}$ avec $\vec{r}_{a0}$ la position immobile, donc constante dans le temps, de l'atome $a$ \`a l'\'equilibre et $\vec{u}_{a}$, l'\'ecart par rapport \`a l'\'equilibre, nous obtenons :
\be
	\sum_{a}m_{a}\dot{\vec{u}}_{a} = \vec{0} \Leftrightarrow \sum_{a}m_{a}\vec{u}_{a} = \vec{cste} = \vec{0} \label{EQ:24_1}
\ee
...
\benn
	\vec{M} = \sum_{a}m_{a}\vec{r}_{a}\wedge\vec{v}_{a} = \sum_{a}m_{a}(\vec{r}_{a0} + \vec{u}_{a})\wedge\dot{\vec{r}}_{a} \approx \sum_{a}m_{a}\vec{r}_{a0}\wedge\dfrac{\mathrm{d}\dot{\vec{u}}_{a}}{\mathrm{dt}}
\eenn
...
\ee
En d\'{e}finissant $\omega_{0}$ comme la fr\'{e}quence des oscillations libres et $\lambda$ comme le \emph{c{\oe}fficient d'amortissement}, tels que :
\be
	\omega_{0}^{2} = \dfrac{k}{m}\text{ et } 2\lambda = \dfrac{\alpha}{m}
\ee
...
\eenn
Trois cas sont alors possibles et sont discut\'{e}s ci-apr\`{e}s.
...
\bea
	...
	& & + \lambda^{2}(c_{1} + c_{2}\mathrm{t})e^{-\lambda\mathrm{t}} \nonumber \\
	& = & 0 \nonumber
\eea
...
\eenn
L'\'{e}quation de Lagrange pour la coordonn\'{e}e $x_{i}$ est alors, en g\'{e}n\'{e}ralisant l'\'{e}quation (\ref{EQ:25_1}) :
\be
...
\benn
	\dfrac{\mathrm{d}E}{\mathrm{dt}} = \dfrac{\mathrm{d}}{\mathrm{dt}}\left(\sum_{i}\dot{x}_{i}\frac{\partial L}{\partial\dot{x}_{i}}\right) - \dfrac{\mathrm{d}L}{\mathrm{dt}} = \sum_{i}\ddot{x}_{i}\dfrac{\partial L}{\partial\dot{x}_{i}} + \sum_{i}\dot{x}_{i}\dfrac{\mathrm{d}}{\mathrm{dt}}\left(\frac{\partial L}{\partial\dot{x}_{i}}\right) - \dfrac{\mathrm{d}L}{\mathrm{dt}}
\eenn
...
Nous allons nous focaliser sur le cas o\`{u} la force ext\'{e}rieure qui provoque les oscillations est p\'{e}riodique.
...
\begin{figure}[htb!]
	\begin{center}
		...
		\caption{Amplitude de la solution (\ref{EQ:26_3}) avec des valeurs du c{\oe}fficient de frottement comprises entre 0.01 et 1.0 et $\omega_{0} = 1$, $f = 2$ et $m = 5$}\label{FIG:5_26}
	\end{center}
\end{figure}
...
\bea
	...
	\Leftrightarrow 2\gamma^{2} + 4\lambda^{2} - 2\omega_{0}^{2} & = & 0 \Leftrightarrow \gamma = \sqrt{\omega_{0}^{2} - 2\lambda^{2}} \nonumber
\eea
...
\eea
Cette forme de relation entre \'{e}nergie dissip\'{e}e, absorb\'{e}e, et la fr\'{e}quence $\epsilon$ est appel\'{e}e \emph{dispersive}. La valeur maximale de $I$ est atteinte quand $\epsilon$ est nul et elle est de :
\benn
...
\eenn
Ainsi, si le c{\oe}fficient d'amortissement $\lambda$ diminue, alors la hauteur de la courbe de r\'{e}sonance devient plus importante mais sa demi-largeur diminue. D\'{e}montrons que la surface sous $I(\gamma)$ est invariable par rapport au c{\oe}fficient d'amortissement $\lambda$.
...
\ee
permettant de conclure que la surface sous $I(\epsilon)$ ne d\'{e}pend pas du c{\oe}fficient d'amortissement du syst\`{e}me.

\section{R\'{e}sonance param\'{e}trique}\label{PAR:27}
...
Maintenant, introduisons la variable $\tau$ d\'{e}finie telle que $m(\mathrm{t})\mathrm{d}\tau = \mathrm{dt}$ avec $m$ une fonction du temps. Cela permet d'\'{e}crire :
\bea
...
\ee
o\`{u} les fonctions $\Pi_{i}(\mathrm{t})$ sont purement p\'{e}riodiques du temps et de p\'{e}riode $T$. Les constantes $\mu_{i}$ doivent \^{e}tre li\'{e}es par une relation d\'{e}termin\'{e}e. Ainsi, pour $i = 2$, l'\'{e}quation (\ref{EQ:27_2}) donne :
\benn
	\begin{cases}
	...
	\end{cases}
\eenn
Cela implique en soustrayant la seconde \`{a} la premi\`{e}re :
\bea
...
\ee
o\`{u} $a(\mathrm{t})$ et $b(\mathrm{t})$ sont des fonctions du temps variant avec des pulsations bien plus faibles que $\omega_{0}$. La solution exacte \`{a} l'\'{e}quation (\ref{EQ:27_8}) contient \'{e}galement des termes dont la fr\'{e}quence est multiple de la quantit\'{e} $2\omega_{0} + \epsilon$, ce qui en fait des infiniments petits d'ordre sup\'{e}rieur et n\'{e}gligeables en premi\`{e}re approximation (voir exercice (\ref{PAR:27_EX1})).
...
\benn
	...
	\end{cases}
\eenn
soient deux \'{e}quations diff\'{e}rentielles lin\'{e}aires. Nous recherchons une paire de solutions telle que $a(\mathrm{t}) = ae^{s\mathrm{t}}$ et $b(\mathrm{t}) = be^{s\mathrm{t}}$. Les deux \'{e}quations pr\'{e}c\'{e}dentes deviennent alors, quelque soit $\mathrm{t}$ :
\benn
	\begin{cases}
...
\ee
qui n'est possible que pour $\left(\frac{\omega_{0}h}{2}\right)^{2} - 4\lambda^{2} > 0$. Il y a donc un seuil d'amplitude au-dessus duquel la r\'{e}sonance n'est pas possible. Cette valeur $h_{k}$ est :
\benn
...
Nous effectuons le d\'{e}veloppement de la fonction de Lagrange jusqu'au troisi\`{e}me ordre, extension du paragraphe (\ref{PAR:23}), en utilisant le d\'{e}veloppement de Taylor en coordonn\'{e}es g\'{e}n\'{e}ralis\'{e}es $q_{i}$ par rapport \`{a} la position d'\'{e}quilibre $q_{i0}$ :
\bea
...
En appliquant la m\'{e}thode des approximations successives, nous allons chercher une solution du type :
\be
...
\eenn
qui peut se r\'{e}quire \`{a}, puisque $(x_{0} + x_{1})^{3}$ est d'ordre sup\'{e}rieur \`{a} 2 :
\benn
...
\eenn
Cela donne finalement car $\omega_{1}^{2}$ et $\cos^{2}a = (cos(2a) + 1)/2$ sont insignifiants :
\benn
...
\benn
	...
	\end{cases}
\eenn
Pour la premi\`{e}re \'{e}quation, la solution est \'{e}videmment $-\alpha a^{2}/(2\omega_{0}^{2})$. Pour la seconde, nous cherchons une solution en $x_{10}\cos(2\omega\mathrm{t})$ qui, une fois inject\'{e}e, donne :
\bea
...
\eea
car $\omega^{2} = (\omega_{0} + \omega_{1})^{2} = \omega_{0}^{2}$ puisque $\omega_{1}$ reste nulle. Nous obtenons finalement :
\be
...
En ajoutant une force ext\'{e}rieure oscillante de fr\'{e}quence $\gamma$ comme celle de l'{e}quation (\ref{EQ:26_1}) et une force de frottement avec un c{\oe}fficient d'amortissement $\lambda$ \`{a} l'{e}quation (\ref{EQ:28_9}), cette derni\`{e}re devient :
\be
	\ddot{x} + 2\lambda\dot{x} + \omega_{0}^{2}x = \frac{f}{m}\cos(\gamma\mathrm{t}) - \alpha x^{2} - \beta x^{3} \label{EQ:29_1}
\ee
D\'{e}sormais au voisinage de la r\'{e}sonance ordinaire, i.e. $\gamma = \omega_{0} + \epsilon$, et en premi\`{e}re approximation, i.e lin\'{e}aire, la relation entre l'amplitude de la solution oscillatoire forc\'{e}e $b$, l'amplitude $f$ et la fr\'{e}quence de la force ext\'{e}rieure $\gamma$ est donn\'{e}e, au voisinage de la r\'{e}sonance, par la formule de (\ref{EQ:26_7}) :
\be
	b = \frac{f}{2m\omega_{0}\sqrt{\epsilon^{2} + \lambda^{2}}} \Leftrightarrow b^{2}(\epsilon^{2} + \lambda^{2}) = \frac{f}{4m^{2}\omega_{0}^{2}} \label{EQ:29_2}
\ee

\'{E}crivons le caract\`{e}re non-lin\'{e}aire des oscillations avec une relation entre leur fr\'{e}quence propre et l'amplitude sous la forme :
\be
	\omega_{0} + \xi b^{2} \label{EQ:29_3}
\ee
o\`{u} $\xi$ est une constante s'exprimant en fonction des c{\oe}fficients d'anharmonicit\'{e}. Par exemple, la relation (\ref{EQ:28_13}) est de ce type avec :
\benn
	\xi = \frac{3\beta}{8\omega_{0}} - \frac{5\alpha^{2}}{12\omega_{0}^{3}}
\eenn
L'expression (\ref{EQ:29_2}) devient dans ce cas, en rempla\c{c}ant $\epsilon$ par son pendant non-lin\'{e}aire, i.e. en rempla\c{c}ant $\gamma = \omega_{0}$ par $\gamma - \omega_{0} - \xi b^{2} = \epsilon - \xi b^{2}$ :
\be
	b^{2}\left[(\epsilon - \xi b^{2})^{2} + \lambda^{2}\right] = \frac{f^{2}}{4m^{2}\omega_{0}^{2}} \label{EQ:29_4}
\ee
qui permet d'en d\'{e}duire $\epsilon$ pour $-2\lambda m\omega_{0}b < f < 2\lambda m\omega_{0}b$ :
\benn
	(\epsilon - \xi b^{2})^{2} = \left(\frac{f}{2m\omega_{0}}\right)^{2} - \lambda^{2} \Leftrightarrow \epsilon = \xi b^{2} \pm \sqrt{\left(\frac{f}{2m\omega_{0}}\right)^{2} - \lambda^{2}}
\eenn

\subsection{Relation entre l'amplitude des oscillations et la fr\'{e}quence de for\c{c}age}

L'\'{e}quation (\ref{EQ:29_4}) est du troisi\`{e}me degr\'{e} par rapport \`{a} $b^{2}$ et ses racines r\'{e}lles d\'{e}terminent l'amplitude des oscillations forc\'{e}es. Ainsi l'{e}quation (\ref{EQ:29_4}) est \'{e}quivalente \`{a} :
\bea
	b^{2}\left(\epsilon^{2} + \xi^{2}b^{4} - 2\epsilon\xi b^{2} + \lambda^{2}\right) & = & \frac{f^{2}}{4m^{2}\omega_{0}^{2}} \nonumber \\
	\Leftrightarrow \xi^{2}b^{6} - 2\epsilon\xi b^{4} + (\epsilon^{2} + \lambda^{2})b^{2} & = & \frac{f^{2}}{4m^{2}\omega_{0}^{2}} \nonumber
\eea
En supposant $f \rightarrow 0$, i.e. un for\c{c}age de faible intensit\'{e}, nous pouvons n\'{e}gliger le terme d'ordre sup\'{e}rieure \`{a} 2 pour $b$ et nous revenons ainsi \`{a} la formule (\ref{EQ:29_2}) avec $b > 0$, soit :
\benn
	b = \frac{f}{2m\omega_{0}\sqrt{\lambda^{2} + \epsilon^{2}}}
\eenn
avec $b_{max} = b(\epsilon = 0) = \frac{f}{2m\omega_{0}\lambda}$.

Consid\'{e}rons maintenant que l'intensit\'{e} de la force ext\'{e}rieure $f$ cro\^{i}t suffisamment pour que seul le terme en $b^{6}$ soit n\'{e}gligeable. L'\'{e}quation caract\'{e}ristique est alors :
\benn
	2\epsilon\xi b^{4} - (\epsilon^{2} + \lambda^{2})b^{2} + \frac{f^{2}}{4m^{2}\omega_{0}^{2}} = 0
\eenn
qui a pour racines en $b^{2}$ :
\benn
	b^{2} = \frac{(\epsilon^{2} + \lambda^{2}) \pm \sqrt{(\epsilon^{2} + \lambda^{2})^{2} - \frac{2\xi\epsilon f^{2}}{m^{2}\omega_{0}^{2}}}}{4\xi\epsilon}
\eenn

\begin{figure}[htb!]
        \begin{center}
                \includegraphics[width=10cm]{chapter_05_paragraph_29}
		\caption{Amplitude $b$ de la solution pr\'{e}c\'{e}dente avec des valeurs de l'amplitude de la force ext\'{e}rieure comprise entre 0.001 et 0.1 et $\lambda = 0.01$, $\omega_{0} = 0.1$, $x = 0.05$ et $m = 0.25$}\label{FIG:5_29}
        \end{center}
\end{figure}

$b^{2}$ est une quantit\'{e} par d\'{e}finition toujours positive. Dans le cas o\`{u} :
\benn
	b^{2} = \frac{(\epsilon^{2} + \lambda^{2}) + \sqrt{(\epsilon^{2} + \lambda^{2})^{2} - \frac{2\xi\epsilon f^{2}}{m^{2}\omega_{0}^{2}}}}{4\xi\epsilon}
\eenn
alors ce n'est r\'{e}alisable que si $\epsilon > 0$. Car $\forall \epsilon \in \mathbb{R}^{*}$, le num\'{e}rateur est toujours positif alors que le d\'{e}nominateur est n\'{e}gatif si $\epsilon < 0$.
Dans l'autre cas, \`{a} savoir :
\benn
	b^{2} = \frac{(\epsilon^{2} + \lambda^{2}) - \sqrt{(\epsilon^{2} + \lambda^{2})^{2} - \frac{2\xi\epsilon f^{2}}{m^{2}\omega_{0}^{2}}}}{4\xi\epsilon}
\eenn
alors $b^{2} > 0$ si et seulement si :
\begin{itemize}
 \item $\epsilon > 0$ et $(\epsilon^{2} + \lambda^{2}) - \sqrt{(\epsilon^{2} + \lambda^{2})^{2} - \frac{2\xi\epsilon f^{2}}{m^{2}\omega_{0}^{2}}} > 0$, ce qui est toujours vrai car $\epsilon^{2} + \lambda^{2} > \sqrt{(\epsilon^{2} + \lambda^{2})^{2} - \frac{2\xi\epsilon f^{2}}{m^{2}\omega_{0}^{2}}}$ puisque $\epsilon > 0$
 \item $\epsilon < 0$ et $(\epsilon^{2} + \lambda^{2}) - \sqrt{(\epsilon^{2} + \lambda^{2})^{2} - \frac{2\xi\epsilon f^{2}}{m^{2}\omega_{0}^{2}}} < 0$, ce qui est toujours vrai car $\epsilon^{2} + \lambda^{2} < \sqrt{(\epsilon^{2} + \lambda^{2})^{2} - \frac{2\xi\epsilon f^{2}}{m^{2}\omega_{0}^{2}}}$ puisque $\epsilon < 0$
\end{itemize}

\begin{figure}[htb!]
        \begin{center}
                \includegraphics[width=15cm]{chapter_05_paragraph_29_book}
		\caption{Graphiques repris du livre}\label{FIG:5_29_BOOK}
        \end{center}
\end{figure}

Il existe une valeur limite de $f$ not\'{e}e $f_{k}$ telle que pour $f < f_{k}$, seule une racine sur les trois de l'\'{e}quation (\ref{EQ:29_4}) est réelle, alors que pour $f > f_{k}$, les trois racines sont bien toutes r\'{e}elles. Ces derni\`{e}res d\'{e}finissent le domaine (BCDE) sur la courbe (c) de la figure (\ref{FIG:5_29_BOOK}). Il y a l\`{a} un \emph{effet de repliement}. Ce domaine permet de le contraindre avec :
\benn
	\begin{cases}
		\epsilon_{B} = \epsilon_{D} \\
		\epsilon_{C} = \epsilon_{E}
	\end{cases}
	\text{, }\frac{\mathrm{d}b}{\mathrm{d}\epsilon}\bigg|_{C} = \frac{\mathrm{d}b}{\mathrm{d}\epsilon}\bigg|_{D} = -\infty
\eenn
Maintenant, d\'{e}rivons par rapport \`{a} $\epsilon$ l'\'{e}quation (\ref{EQ:29_4}) :
\bea
	\frac{\mathrm{d}\left(b^{2}\left[(\epsilon - \xi b^{2})^{2} + \lambda^{2}\right]\right)}{\mathrm{dt}} & = & 0 \nonumber \\
	\Leftrightarrow \frac{\mathrm{d}}{\mathrm{dt}}\left[b^{2}\epsilon^{2} + \xi^{2}b^{6} - 2\xi\epsilon b^{4} + \lambda^{2}b^{2}\right] & = & 0 \nonumber \\
	\Leftrightarrow 2\epsilon b^{2} + 2\epsilon^{2}b\frac{\mathrm{d}b}{\mathrm{d}\epsilon} + 6\xi^{2}b^{5}\frac{\mathrm{d}b}{\mathrm{d}\epsilon} - 2\xi b^{4} - 8\xi\epsilon b^{3}\frac{\mathrm{d}b}{\mathrm{d}\epsilon} + 2\lambda^{2}b\frac{\mathrm{d}b}{\mathrm{d}\epsilon} & = & 0 \nonumber \\
	\Leftrightarrow (2\epsilon^{2}b + 6\xi^{2}b^{5} - 8\xi\epsilon b^{3} + 2\lambda^{2}b)\frac{\mathrm{d}b}{\mathrm{d}\epsilon} & = & -2\epsilon b^{2} + 2\xi b^{4} \nonumber \\
	\Leftrightarrow \frac{\mathrm{d}b}{\mathrm{d}\epsilon} & = & \frac{2b(-\epsilon b + \xi b^{3})}{2b(\epsilon^{2} + 3\xi^{2}b^{4} - 4\xi\epsilon b^{2} + \lambda^{2})} \nonumber \\
	\Leftrightarrow \frac{\mathrm{d}b}{\mathrm{d}\epsilon} & = & \frac{-\epsilon b + \xi b^{3}}{3\xi^{2}b^{4} - 4\xi\epsilon b^{2} + \epsilon^{2} + \lambda^{2}} \nonumber
\eea
Ainsi vouloir r\'{e}soudre l'\'{e}quation $\frac{\mathrm{d}b}{\mathrm{d}\epsilon} = -\infty$ n\'{e}cessite de poser le d\'{e}nominateur du second membre pr\'{e}c\'{e}dent \`{a} 0, soit :
\be
	3\xi^{2}b^{4} - 4\xi\epsilon b^{2} + \epsilon^{2} + \lambda^{2} = 0 \label{EQ:29_5}
\ee
dont les solutions de ces \'{e}galit\'{e}s du second degr\'{e} sont :
\benn
	\epsilon = \frac{1}{2}\left(4\xi b^{2} \pm \sqrt{16\xi^{2}b^{4} - 4(\lambda^{2} + 3\xi^{2}b^{4})}\right) = 2\xi b^{2}\left(1 \pm \sqrt{1 - \frac{\lambda^{2} + 3\xi^{2}b^{4}}{4\xi^{2}b^{4}}}\right)
\eenn
Or $\forall \alpha \in [0;1]$, $1 > \sqrt{1 - \alpha}$, donc les deux valeurs possibles pour $\epsilon$ sont positives. Il existe alors deux points pour lesquels $\frac{\mathrm{d}b}{\mathrm{d}\epsilon} = \infty$.

\`{A} la vue de la forme de la courbe $b(\epsilon)$, voir le graphique (\ref{FIG:5_29_BOOK}), son maximum est atteint pour $\frac{\mathrm{d}b}{\mathrm{d}\epsilon} = 0$, soit :
\benn
	\epsilon b = \xi b^{3} \Leftrightarrow \epsilon = \xi b^{2}
\eenn
valeur pour $\epsilon$ que nous pouvons injecter dans la relation (\ref{EQ:29_4}) pour obtenir :
\bea
	b_{max}^{2}\left[(\xi b_{max}^{2} - \xi b_{max}^{2}) + \lambda^{2}\right] & = & \frac{f^{2}}{4m^{2}\omega_{0}^{2}} \nonumber \\
	\Leftrightarrow b_{max} & = & \frac{f}{2m\omega_{0}\lambda} \label{EQ:29_6}
\eea
qui est \'{e}galement la valeur maximale de la fonction $b(\epsilon)$ issue de la relation (\ref{EQ:29_2}) pour l'approximation lin\'{e}aire.

La pr\'{e}sence d'un domaine de fr\'{e}quence, entre $\epsilon_{B} = \epsilon_{D}$ et $\epsilon_{C} = \epsilon_{E}$, admettant deux amplitudes diff\'{e}rentes pour une fr\'{e}quence donn\'{e}e est une particularit\'{e} remarquable. Lorsque la fr\'{e}quence de la force ext\'{e}rieure $\gamma = \omega_{0} + \epsilon$ augmente, l'amplitude des oscillations forc\'{e}es augmente et suit la courbe $(ABC)$. Le point $C$ est une rupture de l'amplitude qui tombe \`{a} la valeur $b(\epsilon_{E})$ puis elle varie le long de la courbe $(EF)$ lorsque la fr\'{e}quence continue d'augmenter.

Pour le chemin inverse, si la fr\'{e}quence de la force ext\'{e}rieure diminue depuis $F$, alors le point de rupture est $D$ et l'amplitude saute de $D$ \`{a} $B$, puis d\'{e}cro\^{i}t selon $(BA)$.

Pour calculer $f_{k}$, il est n\'{e}cessaire que l'\'{e}quation (\ref{EQ:29_5}) n'ait qu'une seule racine en $\xi b^{2}$. Soit, en d'autres termes que son discriminant soit nul, i.e. :
\benn
	0 = 16\epsilon_{k}^{2} - 12(\epsilon_{k}^{2} + \lambda^{2}) \Leftrightarrow \epsilon_{k}^{2} = 3\lambda^{2}
\eenn
et, de fait, sa racine unique est :
\benn
	\xi b_{k}^{2} = \frac{4\epsilon_{k}}{12} \Leftrightarrow \xi b_{k}^{2} = \frac{2}{3}\epsilon_{k}
\eenn
En reportant $\epsilon_{k}$ et $b_{k}$ dans l'\'{e}quation (\ref{EQ:29_4}), nous pouvons calculer :
\bea
	\frac{2\epsilon_{k}}{3\xi}\left(\frac{1}{9}\epsilon_{k}^{2} + \lambda^{2}\right) & = & \frac{f_{k}^{2}}{4m^{2}\omega_{0}^{2}} \nonumber \\
	\Leftrightarrow \frac{2\lambda}{\sqrt{3}\xi}\left(\frac{1}{3}\lambda^{2} + \lambda^{2}\right) & = & \frac{f_{k}^{2}}{4m^{2}\omega_{0}^{2}} \nonumber \\
	\Leftrightarrow \frac{8\lambda^{3}}{3\sqrt{3}\xi} & = & \frac{f_{k}^{2}}{4m^{2}\omega_{0}^{2}} \nonumber \\
	\Leftrightarrow f_{k}^{2} & = & \frac{32m^{2}\omega_{0}^{2}\lambda^{3}}{3\sqrt{3}\xi} \nonumber
\eea
qui n'est effectivement possible que pour des valeurs $\xi$ positives. Pour g\'{e}n\'{e}raliser, nous pouvons \'{e}crire :
\be
	f_{k}^{2} = \frac{32m^{2}\omega_{0}^{2}\lambda^{3}}{3\sqrt{3}\lvert\xi\rvert} \label{EQ:29_7}
\ee
Ainsi pour des fr\'{e}quences $\omega_{0}$ de la force ext\'{e}rieure proches de la fr\'{e}quence propre du syst\`{e}me, les ph\'{e}nom\`{e}nes de r\'{e}sonance sont particuliers avec l'apparition des oscillations non-lin\'{e}aires.

\subsection{Cas d'une force ext\'{e}rieure dont la fr\'{e}quence diff\`{e}re largement de $\omega_{0}$ : $\gamma \approx \omega_{0}/2$}

Observons maintenant ce qui arrive dans le cas o\`{u} la fr\'{e}quence de la force ext\'{e}rieure diff\`{e}re sensiblement de $\omega_{0}$. Nous choisissons ici $\gamma \approx \omega_{0}/2$ ou encore $\gamma = \omega_{0}/2 + \epsilon$.

Dans l'approximation lin\'{e}aire, i.e. celle du premier ordre, \'{e}tudi\'{e}e dans le paragraph (\ref{PAR:22}) et en appliquant la solution (\ref{EQ:22_4}) sachant que $\omega_{0}^{2} - \gamma^{2} = \omega_{0}^{2} - \omega_{0}^{2}/4 - \epsilon^{2} + 2\omega_{0}\epsilon \approx 3\omega_{0}^{2}/4$ en premi\`{e}re approximation, nous avons :
\benn
	x_{0} = \frac{4f}{3m\omega_{0}^{2}}\cos\left((\omega_{0}/2 + \epsilon)\mathrm{t}\right)
\eenn
Consid\'{e}rons que l'\'{e}quation du mouvement issue de la deuxi\`{e}me approximation telle que $x = x_{0} + x_{1}$, soit :
\benn
	\ddot{x}_{1} + 2\lambda\dot{x}_{1} + \omega_{2}x_{1} + \alpha x_{1}^{2} + \beta x_{1}^{3} = -\alpha x_{0}^{2}
\eenn
En reprenant la solution $x_{0}$ pr\'{e}c\'{e}dente, le second membre devient :
\benn
	-\alpha x_{0}^{2} = -\frac{16f^{2}\alpha}{9m^{2}\omega_{0}^{4}}\cos^{2}\left((\omega_{0}/2 + \epsilon)\mathrm{t}\right) = -\frac{8f^{2}\alpha}{9m^{2}\omega_{0}^{4}}\left(\cos\left((\omega_{0} + 2\epsilon)\mathrm{t}\right) + 1\right)
\eenn
et, en ne gardant que le terme de r\'{e}sonance, nous obtenons pour l'\'{e}quation diff\'{e}rentielle :
\be
	\ddot{x}_{1} + 2\lambda\dot{x}_{1} + \omega_{2}x_{1} + \alpha x_{1}^{2} + \beta x_{1}^{3} = -\frac{8f^{2}\alpha}{9m^{2}\omega_{0}^{4}}\cos\left((\omega_{0} + 2\epsilon)\mathrm{t}\right) \label{EQ:29_8}
\ee
qui ne diff\`{e}re de l'\'{e}quation (\ref{EQ:29_1}) que par le fait que l'amplitude de la force ext\'{e}rieure est d\'{e}sormais proportionnelle \`{a} $f^{2}$ et non plus \`{a} $f$. L'analogie avec l'\'{e}quation du mouvement (\ref{EQ:29_1}) permet de prendre appui sur l'\'{e}quation (\ref{EQ:29_4}) en rempla\c{c}ant $f$ par $-\frac{8f^{2}\alpha}{9m^{2}\omega_{0}^{4}}$ et $\epsilon$ par $2\epsilon$ et nous avons :
\be
	b^{2}\left[(2\epsilon - \xi b^{2})^{2} + \lambda^{2}\right] = \frac{1}{4m^{2}\omega_{0}^{2}}\frac{64\alpha^{2}f^{4}}{81m^{2}\omega_{0}^{8}} = \frac{16\alpha^{2}f^{4}}{81m^{2}\omega_{0}^{10}} \label{EQ:29_9}
\ee

\subsection{Cas d'une force ext\'{e}rieure dont la fr\'{e}quence diff\`{e}re largement de $\omega_{0}$ : $\gamma \approx 2\omega_{0}$}

En utilisant la m\^{e}me m\'{e}thodologie que pr\'{e}c\'{e}demment, nous avons $\omega_{0}^{2} - \gamma^{2}= \omega_{0}^{2} - 4\omega_{0}^{2} - \epsilon^{2} - 4\omega_{0}\epsilon \approx -3\omega_{0}^{2}$ en premi\`{e}re approximation. Soit en appliquant la solution (\ref{EQ:22_4}) :
\benn
	x_{0} = -\frac{f}{3m\omega_{0}^{2}}\cos\left((2\omega_{0} + \epsilon)\mathrm{t}\right)
\eenn
Consid\'{e}rons $x = x_{0} + x_{1}$ dans l'\'{e}quation (\ref{EQ:29_1}) :
\bea
	\ddot{x} + 2\lambda\dot{x} + \omega_{0}^{2}x & = & \frac{f}{m}\cos(\gamma\mathrm{t}) - \alpha x^{2} - \beta x^{3} \nonumber \\
	\Leftrightarrow \ddot{x}_{0} + 2\lambda\dot{x}_{0} + \omega_{0}^{2}x_{0} + \ddot{x}_{1} + 2\lambda\dot{x}_{1} + \omega_{0}^{2}x_{1} & = & \frac{f}{m}\cos(\gamma\mathrm{t}) - \alpha(x_{0}^{2} + x_{1}^{2} + 2x_{0}x_{1}) \nonumber \\
	& & - \beta(x_{0}^{3} + x_{1}^{3} + 3x_{0}^{2}x_{1} + 3x_{0}x_{1}^{2}) \nonumber
\eea
Or $x_{0}$ est solution de l'\'{e}quation :
\benn
	\ddot{x}_{0} + 2\lambda\dot{x}_{0} + \omega_{0}^{2}x_{0} = \frac{f}{m}\cos(\gamma\mathrm{t})
\eenn
Donc nous pouvons en d\'{e}duire la relation suivante :
\benn
	\ddot{x}_{1} + 2\lambda\dot{x}_{1} + \omega_{0}^{2}x_{1} + \alpha x_{1}^{2} + \beta x_{1}^{3} = - \alpha x_{0}^{2} - 2\alpha x_{0}x_{1} - \beta x_{0}^{3} - \beta(3x_{0}^{2}x_{1} + 3x_{0}x_{1}^{2})
\eenn
Les termes $-\alpha x_{0}^{2}$ et $-\beta x_{0}^{3}$ ne peuvent \^{e}tre interpr\^{e}t\'{e}s comme des forces ext\'{e}rieures de r\'{e}sonance comme cela a pu \^{e}tre le cas dans l'\'{e}quation (\ref{EQ:29_8}) car leur fr\'{e}quence sont de l'ordre de $4\omega_{0}$ et de $6\omega_{0}$. Le terme $-2\alpha x_{0}x_{1}$ fait, lui, \'{e}merger une r\'{e}sonance param\'{e}trique car l'\'{e}quation pr\'{e}cedente devient avec l'expression de $x_{0}$ et en ne gardant que ce terme proportionnel \`{a} $x_{0}x_{1}$ :
\be
	\ddot{x}_{1} + 2\lambda\dot{x}_{1} + \omega_{0}^{2}\left[1 - \frac{2\alpha f}{3m\omega_{0}^{4}}\cos\left((2\omega_{0} + \epsilon)\mathrm{t}\right)\right]x_{1} = 0 \label{EQ:29_10}
\ee
Cette derni\`{e}re \'{e}quation est similaire \`{a} l'\'{e}quation (\ref{EQ:27_8}) \`{a} la diff\'{e}rence qu'elle prend en consid\'{e}ration le frottement. Elle conduit \`{a} une instabilit\'{e} dans un intervalle de fr\'{e}quences d\'{e}termin\'{e}. Toutefois, cette \'{e}quation n'est pas suffisante pour d\'{e}terminer l'amplitude r\'{e}sultante des oscillations. En effet, cette amplitude d\'{e}pend des effets de non-lin\'{e}arit\'{e} dont il faut garder les termes :
\be
	\ddot{x}_{1} + 2\lambda\dot{x}_{1} + \omega_{0}^{2}x_{1} + \alpha x_{1}^{2} + \beta x_{1}^{3} = \frac{2f}{3m\omega_{0}^{2}}\cos\left((2\omega_{0} + \epsilon)\mathrm{t}\right)x_{1} \label{EQ:29_11}
\ee
Pour simplifier l'\'{e}tude du probl\`{e}me, posons le second membre tel que :
\benn
	\frac{2f}{3m\omega_{0}^{2}}\cos\left((2\omega_{0} + \epsilon)\mathrm{t}\right) \times b\cos\left((\omega_{0} + \epsilon/2)\mathrm{t} + \delta\right)
\eenn
avec $b$ l'amplitude des oscillations de r\'{e}sonance cherch\'{e}e et $\delta$ un d\'{e}phasage constant. Comme $\cos a\cos b = (\cos(a + b) + \cos(a - b))/2$, le second membre s'\'{e}crit :
\benn
	\frac{2f}{3m\omega_{0}^{2}}\left[\cos\left(3(\omega_{0} + \epsilon/2)\mathrm{t} + \delta\right) + \cos\left((\omega_{0} + \epsilon/2)\mathrm{t} - \delta\right)\right]
\eenn
Le terme de r\'{e}sonance ordinaire par rapport \`{a} $\omega_{0}$ est donc $\frac{2f}{3m\omega_{0}^{2}}\cos\left((\omega_{0} + \epsilon/2)\mathrm{t} - \delta\right)$. Et l'\'{e}quation (\ref{EQ:29_11}) devient alors :
\benn
	\ddot{x}_{1} + 2\lambda\dot{x}_{1} + \omega_{0}^{2}x_{1} + \alpha x_{1}^{2} + \beta x_{1}^{3} = \frac{2f}{3m\omega_{0}^{2}}\cos\left((\omega_{0} + \epsilon/2)\mathrm{t} - \delta\right) - \alpha x_{1}^{2} - \beta x_{1}^{3}
\eenn
qui est, \`{a} la valeur de l'amplitude de la force ext\'{e}rieure pr\`{e}s : $\frac{2f}{3m\omega_{0}^{2}}$ au lieu de $\frac{f}{m}$ et $\epsilon/2$ au lieu de $\epsilon$, identique \`{a} l'\'{e}quation (\ref{EQ:29_1}). Nous faisons donc la substitution dans (\ref{EQ:29_4}) et nous obtenons :
\benn
	b^{2}\left[(\frac{\epsilon}{2} - \xi b^{2})^{2} + \lambda^{2}\right] = \frac{\alpha^{2}f^{2}b^{2}}{36m^{2}\omega_{0}^{6}}
\eenn
qui a pour solution triviale :
\be
	b = 0 \label{EQ:29_12}
\ee
En supposant $b \neq 0$, nous pouvons d\'{e}velopper la relation pr\'{e}c\'{e}dente :
\benn
	(\frac{\epsilon}{2} - \xi b^{2})^{2} + \lambda^{2} = \frac{\alpha^{2}f^{2}b^{2}}{36m^{2}\omega_{0}^{6}b^{2}} \Leftrightarrow (\frac{\epsilon}{2} - \xi b^{2})^{2} = \left(\frac{\alpha f}{6m\omega_{0}^{3}}\right)^{2} - \lambda^{2}
\eenn
En supposant \'{e}galement $\left(\frac{\alpha f}{6m\omega_{0}^{3}}\right)^{2} > \lambda^{2}$, nous avons pour solution :
\bea
	b_{+}^{2} & = & \frac{1}{\xi}\left[\frac{\epsilon}{2} + \sqrt{\left(\frac{\alpha f}{6m\omega_{0}^{3}}\right)^{2} - \lambda^{2}}\right] \label{EQ:29_13} \\
	b_{-}^{2} & = & \frac{1}{\xi}\left[\frac{\epsilon}{2} - \sqrt{\left(\frac{\alpha f}{6m\omega_{0}^{3}}\right)^{2} - \lambda^{2}}\right] \label{EQ:29_14}
\eea

\begin{figure}[htb!]
        \begin{center}
                \includegraphics[width=15cm]{chapter_05_paragraph_29.33}
		\caption{Solutions $b$ (\ref{EQ:29_12}), (\ref{EQ:29_13}) et (\ref{EQ:29_14}) pour $\sqrt{\left(\frac{\alpha f}{6m\omega_{0}^{3}}\right)^{2} - \lambda^{2}} = 1$ et $\xi = 0.2$.}\label{FIG:5_29.33}
        \end{center}
\end{figure}

Concentrons-nous sur le cas $\xi > 0$, sachant que dans le cas inverse il s'agira de prendre l'argumentaire dans le sens oppos\'{e}. La quantit\'{e} $b^{2}$ s'annule si et seulement si :
\benn
	\frac{\epsilon_{+-}}{2} \pm \sqrt{\left(\frac{\alpha f}{6m\omega_{0}^{3}}\right)^{2} - \lambda^{2}} \Leftrightarrow \epsilon_{+-} = \pm \sqrt{\left(\frac{\alpha f}{6m\omega_{0}^{3}}\right)^{2} - \lambda^{2}}
\eenn
Ainsi pour $\epsilon < \epsilon_{-}$, l'amplitude $b$ est nulle, il n'y a pas de r\'{e}sonances et les oscillations de fr\'{e}quence $\approx\omega_{0}$ ne sont pas exit\'{e}es. Pour $\epsilon_{-} < \epsilon < \epsilon_{+}$, il y a deux racines, \`{a} savoir : $b = 0$ et $b = b_{+}$. Cet intervalle correspond au domaine de r\'{e}sonance param\'{e}trique identifi\'{e} dans l'in\'{e}galit\'{e} (\ref{EQ:27_12}). En comparant les relations (\ref{EQ:29_10}) et (\ref{EQ:27_8}), nous avons la correspondance $h = \lvert-\frac{2\alpha f}{3m\omega_{0}^{4}}\rvert$. Or pour l'existence de $\epsilon$, il faut :
\benn
	\left(\frac{\alpha f}{3m\omega_{0}^{3}}\right)^{2} > 4\lambda^{2} \Leftrightarrow \lvert\frac{\alpha f}{3m\omega_{0}^{3}}\rvert > 2\lambda \Leftrightarrow \frac{\omega_{0}h}{2} > 2\lambda \Leftrightarrow h > \frac{4\lambda}{\omega_{0}}
\eenn
Nous retrouvons ici la condition \'{e}nonc\'{e}e \`{a} la fin du paragraphe (\ref{PAR:27}).

Dans cet intervalle, la r\'{e}sonance param\'{e}trique est donc privil\'{e}gi\'{e}e par le syst\`{e}me et de fait, la solution $b = 0$ y est instable.

Pour $\epsilon > \epsilon_{+}$, les trois racines $b \in \{0,b_{-},b_{+}\}$ sont possibles. Dans le cours, il est indiqu\'{e} que la solution $b_{-}$ y est instable. Nous n'avons pas trouvé, encore, le chemin pour justifier ce point.

Suivons le comportement d'un syst\`{e}me initialement en r\'{e}gime nominal des faibles oscillations de fr\'{e}quence $\gamma$ lorsque nous diminuons la fr\'{e}quence de la force ext\'{e}rieure et en nous appuyant sur la figure (\ref{FIG:5_29.33}). Avant d'arriver au point $C$, l'amplitude $b$ est toujours nulle. Puis, il se produit une rupture pour passer sur la branche $(EB)$. La valeur de $\epsilon$ continuant de diminuer, l'amplitude des oscillations d\'{e}cro\^{i}t \'{e}galement jusque z\'{e}ro et le point $B$. Si la fr\'{e}quence augmente ensuite, alors l'amplitude parcoura la branche $(BE)$.

Les cas de r\'{e}sonance envisag\'{e}s ici sont les cas fondamentaux se produisant dans un syst\`{e}me \`{a} oscillations non-lin\'{e}aires. Aux approximations sup\'{e}rieures, des ph\'{e}nom\`{e}nes de r\'{e}sonance apparaissent sur d'autres fr\'{e}quences. Surtout, il doit y avoir r\'{e}sonance pour toute fr\'{e}quence $\gamma$ telle que $n\gamma + m\omega_{0} = \omega_{0}$ pour $(n,m) \in \mathbb{N}$, i.e. telle que $\gamma = p\omega_{0} / q$ avec $(p,q) \in \mathbb{N}$. Toutefois, l'intensit\'{e} des ph\'{e}nom\`{e}nes de r\'{e}sonance ainsi que la largeur des domaines de fr\'{e}quences de ces r\'{e}sonances diminuent tr\`{e}s rapidement avec le degr\'{e} d'approximation. De sorte qu'en r\'{e}alit\'{e}, seules sont observables les r\'{e}sonances pour des fr\'{e}quences $\gamma \approx p\omega_{0} / q$ avec $p$ et $q$ petits.

\section{Mouvement dans un champ \`{a} oscillations rapides}\label{PAR:30}

Ici, nous allonw consid\'{e}rer le mouvement d'une particule soumise simultan\'{e}ment \`{a} l'action d'un champ constant $U$ et d'une force ext\'{e}rieure d\'{e}finie telle que :
\be
	f = f_{1}\cos(\omega\mathrm{t}) + f_{2}\sin(\omega\mathrm{t}) \label{EQ:30_1}
\ee
qui varie avec le temps \`{a} une fr\'{e}quence $\omega$ \'{e}lev\'{e}e et avec $f_{1}$ et $f_{2}$ qui ne d\'{e}pendent que des seules coordonn\'{e}es. Nous entendons par fr\'{e}quence \'{e}lev\'{e}e la condition $\omega \gg 1/T$ o\`{u} $T$ est l'ordre de grandeur de la p\'{e}riode des oscillations si la particule n'\'{e}tait soumise qu'au champ $U$. Nous supposons tout de m\^{e}me que le d\'{e}placement oscillatoire de la particule provoqu\'{e} par $f$, not\'{e} par le suite $\xi$, est petit.

\subsection{Cas unidimensionnel}

Pour simplifier, nous consid\'{e}rons le mouvement lin\'{e}aire dans un champ d\'{e}pendant d'une seule dimension spatiale. L'\'{e}quation du mouvement est alors :
\be
	m\ddot{x} = -\frac{\mathrm{d}U}{\mathrm{d}x} + f \label{EQ:30_2}
\ee
L'hypoth\`{e}se des petits d\'{e}placements provoqu\'{e}s par $f$ nous invite \`{a} \'{e}crire $x(\mathrm{t})$ tel que :
\be
	x(\mathrm{t}) = X(\mathrm{t}) + \xi(\mathrm{t}) \label{EQ:30_3}
\ee
$\xi(\mathrm{t})$ \'{e}tant une oscillation, sa valeur moyenne calcul\'{e}e sur sa p\'{e}riode $2\pi/\omega$ est nulle, alors que $X(\mathrm{t})$ aura vari\'{e} faiblement. \`{A} partir de cette remarque, nous pouvons d\'{e}duire que $\overline{x} = \overline{T}$, i.e. $X$ d\'{e}crit le mouvement non perturb\'{e} moyen de la particule par rapport aux oscillations rapides. La m\'{e}thode de d\'{e}termination de l'\'{e}quation du mouvement a \'{e}t\'{e} d\'{e}crite par Piotr Kapitsa en 1951.

En utilisation $x(\mathrm{t})$ d\'{e}finie par la relation (\ref{EQ:30_4}) dans l'\'{e}quation (\ref{EQ:30_2}), nous \'{e}crivons :
\benn
	m\ddot{x} = -\frac{\mathrm{d}U}{\mathrm{d}x} + f(x,\mathrm{t}) \Leftrightarrow m\ddot{X} + m\ddot{\xi} = -\frac{\mathrm{d}U}{\mathrm{d}(X+\xi)} + f(X+\xi,\mathrm{t})
\eenn
Puisque $\xi$ est petit devant $X$, nous pouvons donc utiliser le d\'{e}veloppement de Taylor au premier ordre tel que :
\benn
	\begin{cases}
		\frac{\mathrm{d}U}{\mathrm{d}(X+\xi)} = \frac{\mathrm{d}U}{\mathrm{d}X} + \xi\frac{\mathrm{d}^{2}U}{\mathrm{d}X^{2}} \\
		f(X+\xi,\mathrm{t}) = f(X,\mathrm{t}) + \xi\frac{\partial f}{\partial X}
	\end{cases}
\eenn
Cela permet d'\'{e}crire l'\'{e}quation du mouvement ainsi :
\be
	m\ddot{X} + m\ddot{\xi} = -\frac{\mathrm{d}U}{\mathrm{d}X} - \xi\frac{\mathrm{d}^{2}U}{\mathrm{d}X^{2}} + f(X,\mathrm{t}) + \xi\frac{\partial f}{\partial X} \label{EQ:30_4}
\ee
Le second membre se compse de deux parties distinctes :
\begin{itemize}
	\item une non perturb\'{e}e $-\frac{\mathrm{d}U}{\mathrm{d}X}$
	\item une oscillante $- \xi\frac{\mathrm{d}^{2}U}{\mathrm{d}X^{2}} + f(X,\mathrm{t}) + \xi\frac{\partial f}{\partial X}$ qui peut \^{e}tre r\'{e}duite \`{a} $f(X,\mathrm{t})$ puisque $\xi$ est par d\'{e}finition petit
\end{itemize}
Ainsi la partie oscillante peut se calculer en r\'{e}solvant :
\be
	m\ddot{\xi} = f(X,\mathrm{t}) \label{EQ:30_5}
\ee
qui peut se d\'{e}composer en :
\benn
	m\ddot{\xi} = f_{1}(X)\cos(\omega\mathrm{t}) + f_{2}(X)\sin(\omega\mathrm{t})
\eenn
qui donne par int\'{e}gration double et en supposant comme nulle les constantees d'int\'{e}gration, nous arrivons \`{a} :
\bea
	m\dot{\xi} & = & \frac{f_{1}(X)}{\omega}\sin(\omega\mathrm{t}) - \frac{f_{2}(X)}{\omega}\cos(\omega\mathrm{t}) \nonumber \\
	\Leftrightarrow m\xi & = & -\frac{f_{1}(X)}{\omega^{2}}\cos(\omega\mathrm{t}) - \frac{f_{2}(X)}{\omega^{2}}\sin(\omega\mathrm{t}) \nonumber \\
	\Leftrightarrow \xi & = & -\frac{f(X,\mathrm{t})}{m\omega^{2}} \label{EQ:30_6}
\eea
Maintenant, calculons la moyenne dans le temps de l'\'{e}quation (\ref{EQ:30_4}) :
\bea
	m\ddot{X} + m\ddot{\xi} & = & -\frac{\mathrm{d}U}{\mathrm{d}X} - \xi\frac{\mathrm{d}^{2}U}{\mathrm{d}X^{2}} + f(X,\mathrm{t}) + \xi\frac{\partial f}{\partial X} \nonumber \\
	m\ddot{X} - f(X,\mathrm{t}) & = & -\frac{\mathrm{d}U}{\mathrm{d}X} - \xi\frac{\mathrm{d}^{2}U}{\mathrm{d}X^{2}} + f(X,\mathrm{t}) + \xi\frac{\partial f}{\partial X} \nonumber
\eea
or $\overline{f} = 0$ donc $\overline{\xi} = 0$ par la relation (\ref{EQ:30_6}). Aussi, comme $U$ n'est pas fonction du temps, alors sa moyenne est \'{e}gale \`{a} sa propre valeur, donc :
\benn
	\begin{cases}
		\overline{\frac{\mathrm{d}U}{\mathrm{d}X}} = \frac{\mathrm{d}U}{\mathrm{d}X} \\
		\overline{\xi\frac{\mathrm{d}^{2}U}{\mathrm{d}X^{2}}} = \overline{\xi}\frac{\mathrm{d}^{2}U}{\mathrm{d}X^{2}} = 0
	\end{cases}
\eenn
Ainsi, la moyenne de l'\'{e}quation (\ref{EQ:30_4}) se r\'{e}duit \`{a} :
\benn
	m\ddot{X} = -\frac{\mathrm{d}U}{\mathrm{d}X} - \overline{\xi\frac{\partial f}{\partial X}}
\eenn
que nous pouvons r\'{e}\'{e}crire comme :
\be
	m\ddot{X} = -\frac{\mathrm{d}U_{eff}}{\mathrm{d}X} \label{EQ:30_7}
\ee
avec $U_{eff}$ l'\'{e}nergie potentielle effective d\'{e}finie comme $U_{eff} = U + \frac{1}{2m\omega^{2}}\overline{f^{2}}$. En effet :
\benn
	\frac{\mathrm{d}U_{eff}}{\mathrm{d}X} = \frac{\mathrm{d}U}{\mathrm{d}X} + \frac{1}{2m\omega^{2}}\overline{\frac{\partial f^{2}}{\partial X}} = \frac{\mathrm{d}U}{\mathrm{d}X} + \frac{1}{m\omega^{2}}\overline{f\frac{\partial f}{\partial X}}
\eenn
et comme $f^{2} = f_{1}^{2}\cos^{2}(\omega\mathrm{t}) + f_{2}^{2}\sin^{2}(\omega\mathrm{t}) + 2f_{1}f_{2}\cos(\omega\mathrm{t})\sin(\omega\mathrm{t})$ alors $\overline{f^{2}} = f_{1}^{2}/2 + f_{2}^{2}/2$. L'\'{e}nergie potentielle effective s'exprime donc ainsi :
\be
	U_{eff} = U + \frac{1}{4m\omega^{2}}(f_{1}^{2} + f_{2}^{2}) \label{EQ:30_8}
\ee

Par d\'{e}rivation de la relation (\ref{EQ:30_6}), nous pouvons \'{e}crire :
\bea
	\dot{\xi} & = & -\frac{\dot{f}}{m\omega^{2}} = -\frac{1}{m\omega^{2}}(-f_{1}\omega\sin(\omega\mathrm{t}) + f_{2}\omega\cos(\omega\mathrm{t})) \nonumber \\
	\Leftrightarrow \dot{\xi}^{2} & = & \frac{1}{m^{2}\omega^{4}}(f_{1}^{2}\omega^{2}\sin^{2}(\omega\mathrm{t}) + f_{2}^{2}\omega^{2}\cos^{2}(\omega\mathrm{t}) - 2f_{1}f_{2}\cos(\omega\mathrm{t})\sin(\omega\mathrm{t})) \nonumber \\
	\Leftrightarrow \overline{\dot{\xi}^{2}} & = & \frac{1}{2m^{2}\omega^{2}}(f_{1}^{2} + f_{2}^{2}) = \frac{2}{m}(U_{eff} - U) \nonumber \\
	\Leftrightarrow U_{eff} & = & U + \frac{m}{2}\overline{\dot{\xi}^{2}} \label{EQ:30_9}
\eea

$\overline{\dot{\xi}^{2}}$ est justement l'expression de l'\'{e}nergie cin\'{e}tique moyenne du mouvement oscillatoire, qui, par d\'{e}finition, est constante par rapport au temps. Le mouvement moyen de la particule par rapport aux oscillations s'effectue comme si, en plus du champ $U$, aggissait un champ suppl\'{e}mentaire constant et d\'{e}pendant du carr\'{e} de l'amplitude d'un champ variable.

\subsection{G\'{e}n\'{e}ralisation \`{a} un nombre quelconque de degr\'{e}s de libert\'{e}}

Reprenons le raisonnement du paragraphe pr\'{e}c\'{e}dent mais en utilisant les coordonn\'{e}es g\'{e}n\'{e}ralis\'{e}es. Alors nous pouvons \'{e}tendre l'\'{e}quation (\ref{EQ:30_2}) comme ceci apr\`{e}s avoir d\'{e}fini $f_{i} = f_{1i}\cos(\omega\mathrm{t}) + f_{2i}\sin(\omega\mathrm{t})$ :
\benn
	\forall i\text{, }a_{i}\ddot{q}_{i} = -\frac{\mathrm{d}U}{\mathrm{d}q_{i}} + \sum_{j}f_{j}
\eenn
En posant $q_{i} = Q_{i} + \xi_{i}$, la relation (\ref{EQ:30_4}) devient :
\benn
	\forall i\text{, }a_{i}\ddot{Q}_{i} + a_{i}\ddot{\xi}_{i} = -\frac{\mathrm{d}U}{\mathrm{d}q_{i}} - \xi_{i}\frac{\mathrm{d}^{2}U}{\mathrm{d}q_{i}^{2}} + \sum_{j}f_{j} + \sum_{j}\xi_{j}\frac{\partial f_{j}}{\partial q_{j}}
\eenn
En moyennant cette derni\`{e}re, nous pouvons d\'{e}velopper tel que :
\benn
	\forall i\text{, }a_{i}\ddot{Q}_{i} = -\frac{\mathrm{d}U}{\mathrm{d}q_{i}} + \overline{\sum_{j}\xi_{j}\frac{\partial f_{j}}{\partial q_{i}}} = -\frac{\mathrm{d}U}{\mathrm{d}q_{i}} + \frac{1}{\omega^{2}}\overline{\sum_{j,k}\frac{f_{j}}{a_{jk}}\frac{\partial f_{k}}{\partial q_{i}}}
\eenn
Pour les m\^{e}mes raisons que pour arriver aux \'{e}quations (\ref{EQ:30_8}) et (\ref{EQ:30_9}), nous pouvons d\'{e}finir une \'{e}nergie potentielle effective telle que :
\be
	U_{eff} = U + \frac{1}{\omega^{2}}\sum_{jk}a_{j,k}^{-1}\overline{f_{j}f_{k}} = U + \sum_{j,k}\frac{a_{jk}}{2}\overline{\dot{\xi}_{j}\dot{\xi}_{k}} \label{EQ:30_10}
\ee
