...
\bea
	...
	\xi^{2}b^{6} - 2\epsilon\xi b^{4} + (\epsilon^{2} + \lambda^{2})b^{2} & = & \frac{f^{2}}{4m^{2}\omega_{0}^{2}} \nonumber
\eea
...
\benn
	...
	\end{cases}
	\text{, }\frac{\mathrm{d}b}{\mathrm{d}\epsilon}\rvert_{C} = \frac{\mathrm{d}b}{\mathrm{d}\epsilon}\rvert_{D} = -\infty
\eenn
...
\ee
dont les solutions de ces \'{e}galit\'{e} du second degr\'{e} sont :
\benn
...
Pour calculer $f_{k}$, il est n\'{e}cessaire que l'\'{e}quation (\ref{EQ:29_5}) n'est qu'une seule racine en $\xi b^{2}$. Soit, en d'autres termes que son discriminant soit nul, i.e. :
\benn
...
\bea
	...
	\Leftrightarrow \ddot{x}_{0} + 2\lambda\dot{x}_{0} + \omega_{0}^{2}x_{0} + \ddot{x}_{1} + 2\lambda\dot{x}_{1} + \omega_{0}^{2}x_{1} & = & \frac{f}{m}\cos(\gamma\mathrm{t}) - \alpha(x_{0}^{2} + x_{1}^{2} + 2x_{0}x_{1}) - \beta(x_{0}^{3} + x_{1}^{3} + 3x_{0}^{2}x_{1} + 3x_{0}x_{1}^{2}) \nonumber
\eea
...
\ee
Cette derni\`{e}re \'{e}quation est similaire \`{a} l'\'{e}quation (\ref{EQ:27_8}) \`{a} la diff\'{e}rence qu'elle prend en consid\'{e}ration le frottement. Elle conduit \`{a} une instabilit\'{e} dans un intervalle de fr\'{e}quences d\'{e}termin\'{e} Toutefois, cette \'{e}quation n'est pas suffisance pour d\'{e}terminer l'amplitude r\'{e}sultante des oscillations. En effet, cette amplitude d\'{e}pend des effets de non-lin\'{e}arit\'{e} dont il faut garder les termes :
\be
...
\eenn
Le terme de r\'{e}sonance ordinaire par rapport \`{a} $\omega_{0}$ est donc $\frac{2f}{3m\omega_{0}^{2}}\cos\left((\omega_{0} + \epsilon/2)\mathrm{t} - \delta\right)$. Et l'\'{e}quation (\ref{EQ:29_11}) devient donc :
\benn
...
\eenn
qui est, \`{a} la valeur de l'amplitude de la force ext\'{e}rieure pr\`{e}s : $\frac{2f}{3m\omega_{0}^{2}}$ au lieu de $\frac{f}{m}$ et $\epsilon/2$ au lieu de $\epsilon$, identique \`{a} l'\'{e}quation (\ref{EQ:29_1}). Faisons donc la substitution dans (\ref{EQ:29_4}) et obtenons :
\benn
