...
Les lois de conservations ne d\'ependent absolument pas de l'esp\`ece d'interaction entre les particules et elles permettent ainsi d'arriver \`a des conclusions pourtant importantes pour les processus m\'ecaniques en jeu.

Pour commencer, choisissons la d\'esint\'egration spontan\'ee, i.e. non provoqu\'ee par des forces ext\'erieures, d'une particule en deux autres particules se d\'epla\c{c}ant apr\`es la d\'esint\'egration ind\'ependamment l'une de l'autre. Sa forme la plus simple est quand elle est consid\'er\'{e}e dans un syst\`eme de r\'ef\'erence o\`u la particule initiale est au repos. La conservation de l'impulsion implique :
\benn
	\vec{p}_{1} + \vec{p}_{2} = \vec{0} \Leftrightarrow \vec{p}_{1} = -\vec{p}_{2}
\eenn
ainsi les particules r\'esultantes de la d\'esint\'egration s'\'eloignent l'une de l'autre avec des impulsions \'egales en norme : $\lVert \vec{p}_{1} \rVert = \lVert \vec{p}_{2} \rVert = p_{0}$ et dirig\'ees en sens oppos\'{e}.
...
\benn
	v_{0}^{2} = v^{2} + V^{2} + 2vV\cos(\langle \vec{v},\vec{V}\rangle) = v^{2} + V^{2} + 2vV\cos(\theta)
\eenn
en se basant sur la figure (\ref{FIG:4_14}) o\`u l'angle $\theta$ est l'angle de d\'eviation dans <<~l~>>. La repr\'esentation g\'eom\'etrique (\ref{FIG:4_14}) montre deux cas distincts :
\begin{itemize}
...
\bea
	\vec{v} & = & \vec{V} + \vec{v}_{0} \Leftrightarrow v^{2} = v_{0}^{2} + V^{2} + 2v_{0}V\cos\theta_{0} \nonumber \\
...
\benn
	\mathrm{d}\left(\cos\theta_{0}\right) = -\sin\theta_{0}\mathrm{d}\theta_{0} = \dfrac{\mathrm{d}(v^{2})}{2v_{0}V}
\eenn
...
Si la particule de masse $m_{2}$ est au repos avant le choc, cela se traduit par $\vec{v}_{2} = \vec{0} = \vec{p}_{2}$. Nous avons aussi :
\benn
...
D\'eterminons ensuite la valeur de la vitesse apr\`es le choc pour les deux particules dans <<~l~>>. Nous avons d\'ej\`a vu que $\vec{v'}_{1} = \vec{v'}_{10} + \dot{\vec{R'}}$ sachant que $\dot{\vec{R'}} = \dot{\vec{R}}$, aussi :
\benn
	{v'}_{1}^{\,2} = {v'}_{10}^{\,2} + {\lVert \dot{\vec{R}} \rVert}^{2} + 2v'_{10}\lVert \dot{\vec{R}} \rVert\cos\theta_{10}
\eenn
...
\ee
De mani\`ere similaire pour $v'_{2}$, nous avons $\vec{v'}_{1} = \vec{v'}_{10} + \dot{\vec{R}}$, soit :
\bea
	{v'}_{2}^{\,2} & = & {v'}_{20}^{\,2} + {\lVert \dot{\vec{R}} \rVert}^{2} + 2v'_{20}\lVert \dot{\vec{R}} \rVert\cos(\langle \vec{v'}_{20},\dot{\vec{R}} \rangle) \nonumber \\
...
\eenn
avec $E_{1} = \dfrac{1}{2}m_{1}v_{1}^{2}$ qui est l'\'energie initiale de la particule incidente, de masse $m_{1}$.
...
\ee
Nous remarquons aussi que $\theta_{2} - \theta_{1} = \frac{\pi}{2}$, i.e. que les deux particules s'éloignent l'une de l'autre sous un angle droit. Enfin, dans ce triangle rectangle, nous pouvons noter :
\be
	\begin{cases}
...
\be
	\varphi_{0} = \bigintsss_{r_{min}}^{+\infty}{\dfrac{\frac{M}{r^{2}}\mathrm{d}r}{\sqrt{2m(E - U(r)) - \frac{M^{2}}{r^{2}}}}} \label{EQ:18_2}
\ee
Dans le cadre d'un mouvement infini, il est commode d'\'ecrire l'\'energie $E$ et le moment cin\'etique $M$ en fonction de $v_{\infty}$, la vitesse de la particule incidente pour $r\rightarrow\infty$, et $\rho$, la \emph{distance de vis\'ee}, i.e., la distance minimale par rapport au centre de force s'il n'y avait pas de champ de force. Par conservation de l'\'energie, et parce que $U(\infty) = 0$, et du moment cin\'etique et en accord avec les d\'efinitions pr\'ec\'edentes, la particule incidente poss\`ede les caract\'eristiques du mouvement suivantes :
\be
...
\be
	\mathrm{d}\sigma = 2\pi\rho(\xi)\Bigg\lvert \dfrac{\mathrm{d}\rho(\xi)}{\mathrm{d}\xi} \Bigg\rvert \dfrac{\mathrm{d}\omega}{2\pi\sin\xi} = \dfrac{\rho(\xi)}{\sin\xi}\Bigg\lvert \dfrac{\mathrm{d}\rho(\xi)}{\mathrm{d}\xi} \Bigg\rvert \mathrm{d}\omega \label{EQ:18_8}
\ee
...
\bea
	...
	\Leftrightarrow \sin^{2}\theta_{1}(m_{1}^{2} + m_{2}^{2}\cos^{2}\xi + 2m_{1}m_{2}\cos\xi) & = & m_{2}^{2}\cos^{2}\theta_{1} - m_{2}^{2}\cos^{2}\theta_{1}\cos^{2}\xi \nonumber \\
	\Leftrightarrow m_{1}^{2}\sin^{2}\theta_{1} + m_{2}^{2}\sin^{2}\theta_{1}\cos^{2}\xi + 2m_{1}m_{2}\sin^{2}\theta_{1}\cos\xi & = & m_{2}^{2}\cos^{2}\theta_{1} - m_{2}^{2}\cos^{2}\theta_{1}\cos^{2}\xi \nonumber \\
	\Leftrightarrow (\sin^{2}\theta_{1} + \cos^{2}\theta_{1})m_{2}^{2}\cos^{2}\xi + 2m_{1}m_{2}\sin^{2}\theta_{1}\cos\xi & = & m_{2}^{2}\cos^{2}\theta_{1} - m_{1}^{2}\sin^{2}\theta_{1} \nonumber \\
	\Leftrightarrow \cos^{2}\xi + 2\dfrac{m_{1}}{m_{2}}\sin^{2}\theta_{1}\cos\xi + \dfrac{m_{1}^{2}}{m_{2}^{2}}\sin^{2}\theta_{1} - \cos^{2}\theta_{1} & = & 0 \nonumber
\eea
...
\benn
	\varphi_{0} = \bigintsss_{r_{min}}^{+\infty}{\dfrac{\frac{\rho}{r^{2}}\mathrm{d}r}{\sqrt{1 - \dfrac{\rho^{2}}{r^{2}} - \dfrac{2\alpha}{mv_{\infty}^{2}r}}}}
\eenn
En posant :
\benn
	\begin{cases}
...
\bea
	\varphi_{0} & = & \bigintss_{x_{min}}^{\frac{\alpha}{mv_{\infty}^{2}\rho}}\dfrac{-\mathrm{d}x}{\sqrt{1 - \rho^{2}\left(\dfrac{x}{\rho} - \dfrac{\alpha}{mv_{\infty}^{2}\rho^{2}}\right)^{2} - \dfrac{2\alpha}{mv_{\infty}^{2}}\left(\dfrac{x}{\rho} - \dfrac{\alpha^{2}}{mv_{\infty}^{2}\rho^{2}}\right)}} \nonumber \\
	& = & \bigintss_{x_{min}}^{\frac{\alpha}{mv_{\infty}^{2}\rho}}\dfrac{-\mathrm{d}x}{\sqrt{1 - \rho^{2}\dfrac{x^{2}}{\rho^{2}} - \dfrac{\rho^{2}\alpha^{2}}{m^{2}v_{\infty}^{2}\rho^{4}} + 2\rho^{2}\dfrac{\alpha x}{mv_{\infty}^{2}\rho^{3}} - \dfrac{2\alpha x}{mv_{\infty}^{2}\rho} + \dfrac{2\alpha^{2}}{m^{2}v_{\infty}^{4}\rho^{2}}}} \nonumber \\
	& = & \bigintss_{x_{min}}^{\frac{\alpha}{mv_{\infty}^{2}\rho}}\dfrac{-\mathrm{d}x}{\sqrt{1 + \dfrac{\alpha^{2}}{m^{2}v_{\infty}^{2}\rho^{2}} - x^{2}}} \nonumber
\eea
D\'esormais, posons :
\benn
	\begin{cases}
...
\benn
	\varphi_{0} = \bigintss_{y_{min}}^{Y}\dfrac{-\sqrt{1 + \left(\dfrac{\alpha}{mv_{\infty}^{2}\rho}\right)^{2}}\mathrm{d}y}{\sqrt{1 + \left(\dfrac{\alpha}{mv_{\infty}^{2}\rho}\right)^{2}}\sqrt{1 - y^{2}}} = \bigintsss_{y_{min}}^{Y}\dfrac{-\mathrm{d}y}{\sqrt{1 - y^{2}}}
\eenn
...
\bea
	...
	\Leftrightarrow \cos\varphi_{0}\sqrt{1 + \left(\dfrac{\alpha}{mv_{\infty}^{2}\rho}\right)^{2}} & = & \dfrac{\alpha}{mv_{\infty}^{2}\rho} \Leftrightarrow \cos^{2}\varphi_{0} = \left(\dfrac{\alpha}{mv_{\infty}^{2}\rho}\right)^{2}(1 - \cos^{2}\varphi_{0}) \nonumber \\
...
\benn
	\left(\dfrac{1}{\tan\theta}\right)^{'} = -\dfrac{\tan'\theta}{\tan^{2}\theta} = -\dfrac{1}{\cos^{2}\theta}\dfrac{\cos^{2}\theta}{\sin^{2}\theta} = -\dfrac{1}{\sin^{2}\theta} \Leftrightarrow \left(\dfrac{1}{\tan(\theta/2)}\right)^{'} = -\dfrac{1}{2\sin^{2}(\theta/2)}
\eenn
nous pouvons calculer :
\bea
	\mathrm{d}\sigma & = & 2\pi\rho(\xi)\Bigg\lvert \dfrac{\mathrm{d}\rho(\xi)}{\mathrm{d}\xi} \Bigg\rvert \mathrm{d}\xi = 2\pi\dfrac{\alpha}{mv_{\infty}^{2}}\dfrac{\cos^{2}(\xi/2)}{\sin^{2}(\xi/2)}\Bigg\lvert -\dfrac{1}{2\sin^{2}(\theta/2)} \Bigg\rvert\dfrac{\alpha}{mv_{\infty}^{2}}\mathrm{d}\xi \nonumber \\
	& = & \pi\left(\dfrac{\alpha}{mv_{\infty}^{2}}\right)^{2}\Bigg\lvert \dfrac{\cos(\xi/2)}{\sin^{3}(\xi/2)} \Bigg\rvert \mathrm{d}\xi \label{EQ:19_2}
\eea
En remarquant que :
\be
...
\be
	\mathrm{d}\sigma_{2} = \Bigg\lvert -2\pi\left(\dfrac{\alpha}{mv_{\infty}^{2}}\right)^{2}\dfrac{\cos(\pi/2 - \theta_{2})}{\sin^{3}(\pi/2 - \theta_{2})} \Bigg\rvert \mathrm{d}\theta_{2} = 2\pi\left(\dfrac{\alpha}{mv_{\infty}^{2}}\right)^{2}\dfrac{\sin\theta_{2}}{\cos^{3}\theta_{2}}\mathrm{d}\theta_{2} = \left(\dfrac{\alpha}{mv_{\infty}^{2}}\right)^{2}\dfrac{\mathrm{d}\omega_{2}}{\cos^{3}\theta_{2}} \label{EQ:19_4}
\ee
...
\bea
	\mathrm{d}\sigma & = & \pi\left(\dfrac{\alpha}{mv_{\infty}^{2}}\right)^{2}\Bigg\lvert \dfrac{\cos(\xi/2)}{\sin^{3}(\xi/2)} \Bigg\rvert \mathrm{d}\xi = \pi\left(\dfrac{\alpha}{mv_{\infty}^{2}}\right)^{2}\Bigg\lvert \dfrac{\cos(\xi/2)\sin(\xi/2)}{\sin^{4}(\xi/2)} \Bigg\rvert \mathrm{d}\xi \nonumber \\
...
\ee
car l'\'energie des particules incidentes est $E_{1} = \frac{m_{1}}{2}v_{\infty}^{2}$
...
\bea
	\mathrm{d}\sigma_{1} & = & \pi\left(\dfrac{\alpha}{mv_{\infty}^{2}}\right)^{2}\Bigg\lvert \dfrac{\cos\theta_{1}}{\sin^{3}\theta_{1}} \Bigg\rvert \cdot 2\mathrm{d}\theta_{1} = 2\pi\left(\dfrac{2\alpha}{m_{1}v_{\infty}^{2}}\right)^{2}\Bigg\lvert \dfrac{\cos\theta_{1}}{\sin^{3}\theta_{1}} \Bigg\rvert \mathrm{d}\theta_{1} \nonumber \\
	& = & 2\pi\left(\dfrac{\alpha}{E_{1}}\right)^{2}\Bigg\lvert \dfrac{\cos\theta_{1}}{\sin^{3}\theta_{1}} \Bigg\rvert \mathrm{d}\theta_{1} = \left(\dfrac{\alpha}{E_{1}}\right)^{2}\Bigg\lvert\dfrac{\cos\theta_{1}}{\sin^{4}\theta_{1}} \Bigg\rvert \mathrm{d}\omega_{1} \label{EQ:19_6}
\eea
avec toujours $E_{1} = \frac{m_{1}}{2}v_{\infty}^{2}$ et $\mathrm{d}\omega_{1} = 2\pi\sin\theta_{1}\mathrm{d}\theta_{1}$.

Si en plus de leur masse, les particules initialement au repos et incidentes sont identiques alors il n'est plus n\'{e}cessaire de distinguer apr\`es la diffusion les particules. Avec $\theta = \theta_{1} = \theta_{2}$ et en utilisant les relations (\ref{EQ:19_4}) et (\ref{EQ:19_6}), nous pouvons calculer la surface efficace telle que :
\bea
...
\benn
	\mathrm{d}r = \sqrt{\mathrm{d}x^{2} + \mathrm{d}y^{2} + \mathrm{d}z^{2}} \Rightarrow \dfrac{\partial r}{\partial y} = \dfrac{1}{2}\dfrac{2y}{\sqrt{\mathrm{d}x^{2} + \mathrm{d}y^{2} + \mathrm{d}z^{2}}\partial y} = \dfrac{y}{r}
\eenn
Nous avons alors :
\benn
...
\be
	\mathrm{d}\sigma = \dfrac{\rho(\theta_{1})}{\theta_{1}}\Bigg\lvert \dfrac{\mathrm{d}\rho(\theta_{1})}{\mathrm{d}\theta_{1}} \Bigg\rvert \mathrm{d}\omega_{1} \label{EQ:20_4}
\ee
